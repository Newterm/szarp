...
public:
	virtual int configure(TUnit* unit, xmlNodePtr node, size_t read, size_t send) = 0;
};

/**client driver operating over serial line*/
class serial_client_driver : public client_driver {
public:
	virtual int configure(TUnit* unit, xmlNodePtr node, size_t read, size_t send, serial_port_configuration& spc) = 0;
};
...
public:
	...
	virtual int configure(TUnit* unit, xmlNodePtr node, size_t read, size_t send, serial_port_configuration&) = 0;
};
...
public:
	...
	int configure(TUnit *unit, xmlNodePtr node, size_t read, size_t send, protocols &_protocols);
	int initialize();
