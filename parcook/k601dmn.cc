...
void K601Daemon::Init(int argc, char *argv[])
{
	...
	if (device->hasAttribute("extra:tcp-ip")) {
		...
		sz_log(2, "Connection address %s, cmd port: %hu", m_id.c_str(), m_cmd_port);

	} else if (device->hasAttribute("extra:tcp-ip")) {
		m_path = device->getAttribute("extra:tcp-ip");
		m_id = m_path;
	} else {
		throw SzException("ERROR!: neither IP nor device path has been specified");
	}

	single = cfg->GetSingle() || args_mgr.get<int>("debug").get_value_or(2) > 8;
...
", cfg->GetLineNumber(), device->getAttribute("path").c_str(), kamsinfo->m_params_count, kamsinfo->delay_between_requests_ms / 1000, kamsinfo->unique_registers_count);
	...
	if ((createSend == NULL) || (createReceive == NULL) || (destroySend == NULL)
			|| (destroyReceive == NULL)) {
		sz_log(1, "Error loading symbols frop prop-plugins.so library: %s",
				dlerror());
		dlclose(plugin);
		exit(1);
	}
