...
int get_serial_port_config(TAttribHolder* unit, serial_port_configuration &spc) {
	...
	if (parity.empty()) {
		...
		dolog(0, "Unsupported parity %s, confiugration invalid!!!", parity.c_str());
		return 1;	
	}
	...
	if (stop_bits.empty()) {
		...
		dolog(0, "Unsupported number of stop bits %s, confiugration invalid!!!", stop_bits.c_str());
		return 1;
	}
	...
	if (char_size.empty()) {
		...
		dolog(0, "Unsupported char size %s, confiugration invalid!!!", char_size.c_str());
		return 1;
	}
	return 0;
}
