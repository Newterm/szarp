...
szParSelectEvent::szParSelectEvent(ArrayOfTParam params, int id, wxEventType eventType)
...
class           szParTreeElem:public wxTreeItemData {
  private:
    TParam * m_param;
  public:
    szParTreeElem(TParam * _param):m_param(_param) {
    } TParam       *getParam() {
	return m_param;
    }
};
...
/*
 * class szParTreeValidator : public wxValidator { public: //virtual bool
 * TransferToWindow() {} virtual bool TransferFromWindow() {} } 
 */
	...
	: wxDialog(parent, id, title)
{
    ...
    par_trct = new wxTreeCtrl(this, ID_TRC_PARS,
    		    ...
    		    wxTR_HAS_BUTTONS | (multiple ? wxTR_MULTIPLE | wxTR_EXTENDED : 0));
    szSetDefFont(par_trct);
    wxString CheckBoxName(_("Show like in draw program"));
    check_box = new wxCheckBox(this,szID_TREESEL, CheckBoxName, wxDefaultPosition);
    ...
    if (showShort) {
	    wxStaticBoxSizer *scut_sizer =
		    new wxStaticBoxSizer(new wxStaticBox(this, -1, _("Shortcut")),
				    wxVERTICAL);
    ...
    if (showDescr) {
	    wxStaticBoxSizer *desc_sizer =
		    new wxStaticBoxSizer(new wxStaticBox(this, -1, _("Description")),
				    wxVERTICAL);
    ...
    if (addStyle && single==FALSE) {
	   ...
	   wxButton * add_button = new wxButton(this, szID_PARSELECTADD, _("Add"),
			   wxDefaultPosition);
	   wxButton * close_button = new wxButton(this, szID_PARSELECTCLOSE, _("Close"),
			   wxDefaultPosition);
...
void szParSelect::SetHelp(const wxString& page) {
	...
	tmp_provider = (szHelpControllerHelpProvider *)szHelpControllerHelpProvider::Get();
...
void
szParSelect::LoadParams()
{

    assert(ipk != NULL);
    par_trct->DeleteAllItems();
    // wxLogMessage("rap: loading parlist (%s)", ipk->GetTitle());
	...
	wxStringTokenizer tkz(wxString(param_it->GetTranslatedName()), _T(":"));
	...
	while (tkz.CountTokens() > 1) {
	    ...
	    for (wxTreeItemId i_id =
		 par_trct->GetFirstChild(node_id, cookie); (i_id > 0);
    ...
    // wxLogMessage("rap: ended parlist");
}

void
szParSelect::SortTree(wxTreeItemId id)
{
#if wxCHECK_VERSION(2,5,0)
  wxTreeItemIdValue cookie;
#else
...
void
szParSelect::OnParSelect(wxTreeEvent & ev)
{
    szParTreeElem  *tpar =
	(szParTreeElem *) par_trct->GetItemData(ev.GetItem());
    if (tpar != NULL) {
	// wxLogMessage("rap_tree: selected %s",
	// tpar->getParam()->GetName());
...
void
szParSelect::OnParChanging(wxTreeEvent & ev)
{
...
{
	if(!par_trct->GetSelection().IsOk() || !par_trct->GetItemData(par_trct->GetSelection())) {
		g_data.m_param = NULL;
		last_param = _T("NULL");
	} else {
		last_param = (
			(szParTreeElem *) par_trct->GetItemData(par_trct->GetSelection()))->
				getParam()->GetName();
	}
...
void
szParSelect::OnAddClicked(wxCommandEvent &ev)
{
	...
	if (m_is_multiple && single == FALSE) {
		...
		for (int i = 0; i < count; i++) {
			szParTreeElem *pte = (szParTreeElem*)par_trct->GetItemData(ids.Item(i));
			if (pte)
		...
		szParTreeElem *pte = (szParTreeElem*)par_trct->GetItemData(id);
		if (pte)
			params.Add(pte->getParam());
	}
...
void
szParSelect::OnCloseClicked(wxCommandEvent &ev)
{
...
//	EVT_TREE_ITEM_ACTIVATED(ID_TRC_PARS, szParSelect::OnAddTreeClicked)
