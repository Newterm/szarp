...
class ProgressFrame : public wxDialog {
	...
	std::set<std::wstring> m_synced_prefixes;

	std::set<std::wstring> m_to_sync;
...
class SSCTaskBarItem : public szTaskBarItem {
	...
	/**Handles event generated by "Synchronize given database by prefix" menu item,
	 * shows dialog for databases selection and starts sybchronization*/
	void OnSyncPrefix(wxCommandEvent &event);
