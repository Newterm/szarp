...
EkstraktorWidget::EkstraktorWidget(std::wstring ipk_prefix, wxString * geometry)
{
    ...
    wxProgressDialog* prog = new wxProgressDialog(_("SZARP Extractor v. 3.0."), 
						    wxString(_("Starting program")) + _T("        "),
						    101, NULL);
	...
	wxMessageBox(
		wxString::Format(_T("%s %s\n%s"), 
			_("Cannot load configuration for prefix: "),
			ipk_prefix.c_str(),
			_("It can be a prefix problem")),
		wxString(_("Extractor: Cannot access configuration")),
		wxOK | wxICON_ERROR);
	return;
    }
    ...
    number_of_params = 0;
    int i = 0;
    int max = ipk->GetParamsCount() + ipk->GetDefinedCount() + ipk->GetDrawDefinableCount();
    
    TParam* prm = ipk->GetFirstParam();
    if (NULL == prm) {
	delete prog;
	wxMessageBox(
		wxString::Format(_("Corrupted config %s\nNo params found"),
			    ipk_prefix.c_str()),
	...
	if (first_date < 0) {
	    first_date = szb_search_data(szbase_buffer, prm, 
				tmp_date, /* from */
				-1, /* to - all data */
				1 /* search right */
				);
		...
		tmp_date =  szb_search_data(szbase_buffer, prm, tmp_date, first_date, 1);
		if (tmp_date < first_date) {
		    first_date = tmp_date;
		}
	    }
	...
	if (last_date < 0) {
	    last_date = szb_search_data(szbase_buffer, prm, tmp_date, first_date, -1);
	}
	else {
	    if (tmp_date > last_date) {
		tmp_date = szb_search_data(szbase_buffer, prm, tmp_date, last_date, -1);
		if (tmp_date > last_date) {
		    last_date = tmp_date;
		}
	    }
	...
	delete prog;
	wxMessageBox(wxString::Format(_("Database empty or missing.\nNo params found."),
			    ipk_prefix.c_str()),
		    wxString(_("Extractor: Cannot load parameters")),
		    wxOK | wxICON_ERROR);
	return;
	...
	new EkstraktorMainWindow(this, wxPoint(x, y), wxSize(width, height));
    ...
    wxString paramsnum =
	wxString::Format(_T("%d"), GetNumberOfParams());
    
    mainWindow->SetStatusText(paramsnum + wxString(_(" parameters loaded")));
    is_ok = 1;
}
