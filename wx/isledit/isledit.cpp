...
{
	if ((p->GetType() == TParam::P_REAL) || (p->GetType() == TParam::P_COMBINED))
		return 0;
	return 1;
}

ISLFrame::ISLFrame(wxWindow* parent, int id, const wxString& title, 
		TSzarpConfig *ipk,
		xmlNodePtr _element,
		const wxPoint& pos, const wxSize& size, long style):
    wxFrame(parent, id, title, pos, size, wxCAPTION|wxCLOSE_BOX|wxTAB_TRAVERSAL),
    element(_element)
{
    parameter_txtctrl = new wxTextCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_READONLY|wxTAB_TRAVERSAL);
    ...
    content_rb = new wxRadioBox(this, szID_RADIO, _("Content disposition"), wxDefaultPosition, wxDefaultSize, 2, content_rb_choices, 2, wxRA_SPECIFY_ROWS|wxTAB_TRAVERSAL);
    ...
    combo_box_1 = new wxComboBox(this, wxID_ANY, wxT(""), wxDefaultPosition, wxDefaultSize, 0, combo_box_1_choices, wxCB_DROPDOWN|wxCB_READONLY|wxCB_SORT|wxTAB_TRAVERSAL);
    ...
    xmlChar *prop = xmlGetNsProp(element, (xmlChar*) "uri", (xmlChar*) ISL_URI);
    if (prop) {
    ...
    for (xmlAttrPtr i = element->properties; i; i = i->next) {
	    if (i->ns || !strcmp((const char *)i->name, "id")) continue;
...
void ISLFrame::doOk(wxCommandEvent &event)
{
    ...
    if (parameter_txtctrl->GetValue().length() > 0) {
	    xmlNsPtr ns = xmlSearchNsByHref(element->doc, element, (xmlChar *) ISL_URI);
	    if (!ns) {
		    ...
		    assert (n != NULL);
		    ns = xmlNewNs(n, (xmlChar*) ISL_URI, (xmlChar*) "isl");
		    if (!ns) {
//			    fprintf(stderr, "%s\n", szConv_wx2fs(wxString(_("Error in XML document: ISL namespace is incorrectly defined"))) );
	    ...
	    if (content_rb->GetSelection() == 1) {
		    xmlSetNsProp(element, ns, (xmlChar *) "target",
				    (SC::S2U(combo_box_1->GetValue())).c_str());
...
{
    event.Skip();
//    fprintf(stderr, "%s\n", szConv_wx2fs(wxString(_("Operation canceled by user"))));
...
void ISLFrame::do_layout()
{
    ...
    sizer_6->Add(parameter_bt, 0, wxRIGHT|wxBOTTOM|wxADJUST_MINSIZE|wxALIGN_RIGHT, 10);
    sizer_5->Add(sizer_6, 0, wxEXPAND, 0);
//    sizer_5->Add(parameter_bt, 0, wxALL|wxALIGN_RIGHT|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 10); 
    sizer_5->Add(content_rb, 0, wxLEFT|wxRIGHT|wxBOTTOM|wxEXPAND|wxADJUST_MINSIZE, 10);
    grid_sizer_1->Add(attribute_lb, 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 0);
    grid_sizer_1->Add(combo_box_1, 0, wxALL|wxEXPAND|wxADJUST_MINSIZE, 0);
    grid_sizer_1->Add(scale_lb, 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 0);
    grid_sizer_1->Add(scale_ctrl, 0, wxADJUST_MINSIZE, 0);
    grid_sizer_1->Add(shift_lb, 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 0);
    ...
    sizer_9->Add(ok_bt, 1, wxLEFT|wxRIGHT|wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 10);
    sizer_9->Add(cancel_bt, 1, wxLEFT|wxRIGHT|wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 10);
    sizer_9->Add(help_bt, 1, wxLEFT|wxRIGHT|wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxADJUST_MINSIZE, 10);
...
bool ISLEditor::OnInit()
{
	...
	parser.AddOption(_T("f"), _T("file"), _("IPK file to use"), wxCMD_LINE_VAL_STRING,
	...
	parser.Found(_T("f"), &str);
	if (ipk->loadXML(std::wstring(str))) {
//		fprintf(stderr, "%s", szConv_wx2fs(wxString(_("Error loading configration file "))));
//		fprintf(stderr, " %s\n", szConv_wx2fs(str));
		fprintf(stderr, "%s", SC::S2A(wxString(_("Error loading configration file "))).c_str());
		fprintf(stderr, " %s\n", SC::S2A(str).c_str());
		exit(1);
	}
     if (!parser.Found(_T("i"), &str)) {
//	     fprintf(stderr, "%s\n", szConv_wx2fs(wxString(_("You must select some element"))) );
	     fprintf(stderr, "%s\n", SC::S2A(wxString(_("You must select some element"))).c_str() );
	     exit(1);
     }

     xmlDocPtr input = xmlParseFile(SC::S2A(parser.GetParam()).c_str());
     if (!input) {
	     //fprintf(stderr, "%s\n", szConv_wx2fs(wxString(_("Error parsing input XML file"))) );
	     fprintf(stderr, "%s\n", SC::S2A(wxString(_("Error parsing input XML file"))).c_str() );
	     exit(1);
     }
     xmlXPathContextPtr xpath_ctx = xmlXPathNewContext(input);
     wxString id = _T("//*[@id='") + str + _T("']"); 
     xmlNodePtr element = uxmlXPathGetNode((xmlChar*)SC::S2U(id).c_str(), xpath_ctx);
     if (!element) {
	     //fprintf(stderr, "%s\n", szConv_wx2fs(wxString(_("Selected element not found in XML file"))) );
	     fprintf(stderr, "%s\n", SC::S2A(wxString(_("Selected element not found in XML file"))).c_str() );
	     exit(1);
     }
     ...
     wxString title = wxString(_("ISL Editor: editing <")) + wxString(SC::U2S(element->name)) +
	     _T(">, id ") + str;
