...
struct ttyp_dev {
	...
	wait_queue_head_t slave_open;

	wait_queue_head_t tty_open;
	wait_queue_head_t tty_throttled;
...
static int ttyps_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg);
static ssize_t ttyps_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos);
static ssize_t ttyps_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos);
...
static ssize_t ttypm_write(struct tty_struct *tty, const unsigned char *buf, int count);
static int ttypm_write_room(struct tty_struct *tty);
static void ttypm_set_termios(struct tty_struct *tty, struct ktermios *old_termios);
...
ssize_t ttyps_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	while (kfifo_len(dev->slave_buf) == 0) {
		...
		if (wait_event_interruptible(dev->slave_open, (kfifo_len(dev->slave_buf) > 0)))
			return -ERESTARTSYS;
...
ssize_t ttyps_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos) {
	struct ttyp_dev *dev = filp->private_data;
	...
	while (dev->tty == NULL || test_bit(TTY_THROTTLED, &dev->tty->flags)) {
		...
		if (dev->tty == NULL) {
			...
			if (wait_event_interruptible(dev->tty_open, dev->tty != NULL))
				return -ERESTARTSYS;
		} else {
			spin_unlock(&dev->ttyp_mutex);
			if (wait_event_interruptible(dev->tty_throttled, test_bit(TTY_THROTTLED, &dev->tty->flags) == 0))
				return -ERESTARTSYS;
		}
		spin_lock(&dev->ttyp_mutex);
	}
...
static int ttyps_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
		...
		case TIOCMSET: {
			unsigned char flags;
			if (copy_from_user(&flags, (unsigned char*) arg, sizeof(flags)) == 0) {
				spin_lock(&dev->ttyp_mutex);
				dev->modem_flags = (flags & (TIOCM_CD | TIOCM_RNG | TIOCM_DSR | TIOCM_CTS))
						| (dev->modem_flags & ~((TIOCM_CD | TIOCM_RNG | TIOCM_DSR | TIOCM_CTS)));
				spin_unlock(&dev->ttyp_mutex);
				return 0;
			} else
				return -EFAULT;
	...
	switch (cmd) {
		...
		case TCGETA:
			mutex_lock(&real_tty->termios_mutex);
			if (kernel_termios_to_user_termio((struct termio __user *)arg, real_tty->termios))
				ret = -EFAULT;
			mutex_unlock(&real_tty->termios_mutex);
			break;
	}
...
static unsigned int ttyps_poll(struct file* filp, poll_table *wait) {
	...
	poll_wait(filp, &dev->slave_open, wait);
	poll_wait(filp, &dev->tty_open, wait);
	poll_wait(filp, &dev->tty_throttled, wait);
...
static int ttypm_open(struct tty_struct *tty, struct file *filp) {
	...
	wake_up_interruptible(&dev->tty_open);
...
static void ttypm_release(struct tty_struct *tty, struct file *filp) {
	...
	spin_unlock(&dev->ttyp_mutex);
	wake_up_interruptible(&dev->tty_open);
	wake_up_interruptible(&dev->tty_throttled);
}
...
static void ttypm_set_termios(struct tty_struct *tty, struct ktermios *old_termios) {
	...
	wake_up_interruptible(&dev->slave_open);
}
...
static ssize_t ttypm_write(struct tty_struct *tty, const unsigned char *buf, int count) {
	...
	while (written < count) {
		...
		kfifo_put(dev->slave_buf, (unsigned char*) buf + written, towrite);
...
static int ttypm_tiocmset(struct tty_struct *tty, struct file* filp, unsigned int set, unsigned int clear) {
	struct ttyp_dev *dev = tty->driver_data;
...
static int ttyp_device_init(struct ttyp_dev *dev, int index)
{
	...
	init_waitqueue_head(&dev->slave_open);
	init_waitqueue_head(&dev->tty_open);
	init_waitqueue_head(&dev->tty_throttled);
	...
	tty_register_device(tty_proxy_driver, index, NULL);
...
int ttyp_init(void)
{
	...
	tty_proxy_driver->init_termios.c_cflag = B0 | CS8 | CREAD | HUPCL | CLOCAL;
	...
	ttyp_devices = kmalloc(ttyp_devs_count * sizeof(struct ttyp_dev), GFP_KERNEL);
	if (!ttyp_devices) {
		result = -ENOMEM;
		goto no_mem;
	}
