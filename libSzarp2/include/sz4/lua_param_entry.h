...
template<class types> class lua_caluclate {
	...
	TParam* param;
	bool m_param_prepared;	
public:
	lua_caluclate(base_templ<types>* base, TParam* param) : m_base(base), m_param(param), m_initialized(false), m_lua_param_reference(param->GetLuaParamNoRef()) {
		m_ready = m_base->lua_interpreter().prepare_param(param);
	}
	...
	std::pair<double, bool> calculate_value(second_time_t time, SZARP_PROBE_TYPE probe_type) {
		...
		double value = m_base->lua_interpreter().calculate_value(time, probe_type, 0);
		return std::make_pair(value, stack_top.value());
	}

	~lua_caluclate() {
		if (m_ready) {
			m_base->lua_interpreter().pop_prepared_param();
		}
	}
};
