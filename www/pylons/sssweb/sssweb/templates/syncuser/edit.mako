...
${h.link_to('Enable key', url = h.url_for(action = 'reset_key', id = c.user['name']))}
${h.link_to('Disable key', url = h.url_for(action = 'disable_key', id = c.user['name']))}
% elif c.user['hwkey'] == '0':
Hwkey ${h.text('hwkey_text', value = 'Disabled', disabled = True)}</p>
${h.link_to('Enable key', url = h.url_for(action = 'reset_key', id = c.user['name']))}
${h.link_to('Turn off key', url = h.url_for(action = 'turnoff_key', id = c.user['name']))}
% elif c.user['hwkey'] == '-1':
...
${h.link_to('Disable key', url = h.url_for(action = 'disable_key', id = c.user['name']))}
${h.link_to('Reset key', url = h.url_for(action = 'reset_key', id = c.user['name']))}
${h.link_to('Turn off key', url = h.url_for(action = 'turnoff_key', id = c.user['name']))}
% endif
</p>
